# Georgia Supreme Court Docket Scraper Design
...
| Entry | Param | Purpose |
|-------|-------|---------|
| `get_all_dockets` | none (default) | Sweeps `S21`–`S26` prefix queries (the rolling 5-year window from "today's" calendar year going back five) and yields a detail request per unique case number |
| `get_dockets_after` | `watermark: str` (a case number cursor) | Incremental run — issues `CaseNumber GREATER_THAN <watermark>` and yields a detail request per case lexicographically after the cursor. Catches newly **docketed** cases only; updates to existing cases are not detectable via the search API |
| `fetch_docket_by_number` | `case_number: str` | Direct lookup against `/public-docket/case/{caseNumber}`; useful when CourtListener already knows a docket number and just wants a refresh |

The `get_all_dockets` entry derives its year list from the current date
...
```
get_all_dockets ──▶ parse_search_results ──┐
                                            ├─▶ parse_case_detail ──▶ ParsedData(GaScDocket)
fetch_docket_by_number ─────────────────────┘
```

- `parse_search_results` reads the JSON list returned by the prefix query
  and yields one detail request per `caseNumber`, deduped on the docket id.
- `parse_case_detail` reads the per-case JSON object, splits attorneys and
  filings into nested models, and yields a single `GaScDocket`.
