...
- ``appellate.nccourts.org/dockets.php?…&pdf=1`` — rich HTML docket
  sheet (the primary record). Both `nc` and `ncctapp` cases share the
  same docket-sheet layout, so a single ``NCAppealsDocket`` model
  serves both courts.
"""
...
class NCAppealsParty(ScrapedData):
    ...
    role: str | None = None
    """Role text (e.g. ``Defendant-Appellant``,
    ``Plaintiff-Appellee``)."""
...
class NCAppealsDocketEntry(ScrapedData):
    ...
    text. Both are merged into this model.
    """

    number: int
    """Ordinal index as printed (``(1)``, ``(2)``, …)."""
...
class NCAppealsLowerCourt(ScrapedData):
    ...
    judge: str | None = None
    """Lower-court judge's name."""

    case_number: str | None = None
    """Trial-court docket number (e.g. ``21CR004309-590``)."""
...
class NCAppealsDocument(ScrapedData):
    ...
    ``docket_id`` + ``date_filed`` + ``document_type``.
...
class NCAppealsDocket(ScrapedData):
    ...
    """Close date when ``case_closed`` is True."""
