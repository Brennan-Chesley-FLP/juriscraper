"""Pydantic models for TR Portal (Thomson Reuters C-Track) API responses.

These models validate the structure of JSON API responses from TR Portal
public portal APIs. They are shared across state deployments (Alabama,
Oregon, etc.) and cover the common API endpoints.
...
Field types use unions (e.g., int | str) where different state
deployments return different types for the same field.
"""
...
class PublicationItem(BaseModel):
    ...
    like title, decision, documents come from the detail endpoint.
    """
...
class SearchCaseHeader(BaseModel):
    ...
    Note: courtID is int in some deployments (Alabama) and str in
    others (Oregon). Use int | str to handle both.
    """
...
class DetailCaseHeader(BaseModel):
    ...
    Note: courtID is int in some deployments and str in others.
    """
...
class DocketEntryHeader(BaseModel):
    ...
    Note: documentCount is int in some deployments and str in others.
    """
    ...
    documentCount: int | str | None = None
...
class SubmittedByEntry(BaseModel):
    ...
    Alabama nests actors under party/attorney keys.
    Oregon puts partyActorInstance directly at this level.
    """
