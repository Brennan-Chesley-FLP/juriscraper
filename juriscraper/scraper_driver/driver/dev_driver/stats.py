...
async def get_queue_stats(db: aiosqlite.Connection) -> QueueStats:
    ...
    cursor = await db.execute(SQL.SELECT_QUEUE_STATS_BY_STATUS)
    ...
    cursor = await db.execute(SQL.SELECT_QUEUE_STATS_BY_CONTINUATION)
    rows = await cursor.fetchall()
...
async def get_throughput_stats(db: aiosqlite.Connection) -> ThroughputStats:
    ...
    """
    # Get completed count and time range
    cursor = await db.execute(SQL.SELECT_THROUGHPUT_STATS)
    ...
    if row and row[0] > 0:
        ...
        if row[1] and row[2]:
            cursor = await db.execute(
                SQL.SELECT_DURATION_SECONDS, (row[2], row[1])
            )
...
async def get_compression_stats(db: aiosqlite.Connection) -> CompressionStats:
    ...
    """
    cursor = await db.execute(SQL.SELECT_COMPRESSION_STATS)
    row = await cursor.fetchone()
...
async def get_result_stats(db: aiosqlite.Connection) -> ResultStats:
    ...
    """
    cursor = await db.execute(SQL.SELECT_RESULT_STATS)
    row = await cursor.fetchone()
    ...
    # Get counts by type
    cursor = await db.execute(SQL.SELECT_RESULT_STATS_BY_TYPE)
    rows = await cursor.fetchall()
...
async def get_error_stats(db: aiosqlite.Connection) -> ErrorStats:
    ...
    """
    cursor = await db.execute(SQL.SELECT_ERROR_STATS)
    row = await cursor.fetchone()
    ...
    # Get counts by type
    cursor = await db.execute(SQL.SELECT_ERROR_STATS_BY_TYPE)
    rows = await cursor.fetchall()
    for error_type, count in rows:
        stats.by_type[error_type] = count

    # Get counts by continuation (via joined requests)
    cursor = await db.execute(SQL.SELECT_ERROR_STATS_BY_CONTINUATION)
    rows = await cursor.fetchall()
...
async def get_stats(db: aiosqlite.Connection) -> DevDriverStats:
    ...
    """
    # Get run metadata
    cursor = await db.execute(SQL.SELECT_RUN_METADATA)
    row = await cursor.fetchone()
    ...
    return DevDriverStats(
        ...
        throughput=await get_throughput_stats(db),
        compression=await get_compression_stats(db),
        results=await get_result_stats(db),
        errors=await get_error_stats(db),
