...
from juriscraper.sd.state.common.ctrack import SOFT_404_MARKER, parse_mmddyyyy
...
class NevadaScraper(BaseScraper[NvDocket | NvDocument | NvUnavailableCase]):
    ...
    def _parse_info_header(self, page: PageElement) -> dict[str, str]:
        ...
        caption_cells = page.query_xpath(
            "//td[normalize-space(.)='Short Caption:']",
            "Short Caption label",
            min_count=0,
            max_count=1,
        )
        if not caption_cells:
            return result
        table_rows = caption_cells[0].query_xpath(
            "./ancestor::table[1]//tr", "info table rows", min_count=0
        )
        for row in table_rows:
            cells = row.query_xpath("./td", "info row cells", min_count=0)
    ...
    def _parse_parties(self, page: PageElement) -> list[NvParty]:
        ...
        role_headers = page.query_xpath(
            "//td[normalize-space(.)='Role']",
            "Party Role header",
            min_count=0,
            max_count=1,
        )
        if not role_headers:
            return parties
        header_rows = role_headers[0].query_xpath(
            "./ancestor::tr[1]", "party header row", min_count=1, max_count=1
        )
        if not header_rows:
            return parties
    ...
    def _parse_related_cases(self, page: PageElement) -> list[NvRelatedCase]:
        ...
        label_cells = page.query_xpath(
            "//td[normalize-space(.)='Related Case(s):']",
        ...
        if not label_cells:
            return related
        value_cells = label_cells[0].query_xpath(
        ...
        links = value_cells[0].find_links(
            ".//a", "related case links", min_count=0
        )
        for link in links:
    ...
    def _parse_docket_entries(
        ...
        title_cells = page.query_xpath(
            "//td[normalize-space(.)='Docket Entries']",
        ...
        if not title_cells:
            return entries
        title_rows = title_cells[0].query_xpath(
            "./ancestor::tr[1]",
        ...
        data_rows = title_rows[0].query_xpath(
            "./following-sibling::tr", "docket rows", min_count=0
        )
        for row in data_rows:
