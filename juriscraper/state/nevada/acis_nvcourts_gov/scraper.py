...
Scrapes docket and oral argument data from the Nevada Supreme Court and
Court of Appeals via the Thomson Reuters C-Track Appellate Case
Information System (ACIS) at acis.nvcourts.gov.
...
- Dockets/Cases:
  - API: https://acis-api.nvcourts.gov/courts/cms/cases
  - Portal: https://acis.nvcourts.gov/portal/search/case

- Oral Arguments (Calendar):
  - API: https://acis-api.nvcourts.gov/courts/cms/events
  - Portal: https://acis.nvcourts.gov/portal/search/calendar
...
Dockets Flow (dockets_by_filing_date):
  ...
  6. parse_documents_list -> queues per-document archive downloads, yields NevDocket
  7. parse_document_download -> emits a NevDocument per archived file
...
Oral Arguments Flow (oral_arguments_by_argument_date):
  ...
  3. parse_event_hearings -> parses cases, yields NevOralArgument objects

Nevada's ACIS portal does not expose a publications endpoint for opinion
release lists (the endpoint returns zero results for both courts), so there
is no opinions flow; dispositions surface as docket entries.
"""
...
class NevadaAcisScraper(
    TRPortalMixin,
    BaseScraper[NevDocket | NevDocument | NevOralArgument],
):
    """Scraper for Nevada appellate court data.

    Scrapes dockets and oral argument information from Nevada's C-Track
    Appellate Case Information System (ACIS).
    ...
    court_url: ClassVar[str] = "https://acis.nvcourts.gov/"
    ...
    # === Model classes ===
    DOCKET_CLASS: ClassVar[type] = NevDocket
    DOCKET_ENTRY_CLASS: ClassVar[type] = NevDocketEntry
    DOCUMENT_CLASS: ClassVar[type] = NevDocument
    ORAL_ARGUMENT_CLASS: ClassVar[type] = NevOralArgument
    ...
    def parse_dockets_search(
        ...
        ScraperYield[NevDocket | NevDocument | NevOralArgument], None, None
    ]:
    ...
    def parse_document_download(
        ...
        accumulated_data: dict,
    ) -> Generator[ScraperYield[NevDocument], None, None]:
        """Emit a NevDocument record for an archived file."""
