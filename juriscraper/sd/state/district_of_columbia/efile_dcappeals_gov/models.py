...
class DCAppDocketEntry(ScrapedData):
    ...
    the case; each row is one filing or court-side event.
    """
...
class DCAppParty(ScrapedData):
    ...
    Name, IFP flag, Attorneys, Arguing Attorney, E-Filer flag.
    """
...
class DCAppDocket(ScrapedData):
    ...
    opening_event_date: date | None = None
    record_completed_date: date | None = None
    briefs_completed_date: date | None = None
    argued_submitted_date: date | None = None
    mandate_issued_date: date | None = None
...
class DCAppDocument(ScrapedData):
    ...
    Yielded as a top-level record so consumers can join back to the
    parent ``DCAppDocket`` via ``(docket_id, event_id)`` or to a
