...
- ``fetch_docket(docket_id: str)``                  — ad-hoc single-case lookup
- ``fetch_docket_speculative(case_id: YearlySpeculativeRange)`` — bulk year scan
...
class OhioSupremeCourtScraper(BaseScraper[_Yield]):
    ...
    Speaks the site's ``Ajax.ashx`` JSON API directly with httpx. Emits one
    :class:`OhioSupremeCourtDocket` per case and an
    :class:`OhioSupremeCourtDocument` per archived PDF.
    """
    ...
    def fetch_docket(self, docket_id: str) -> Generator[Request, None, None]:
        """Fetch a single docket by its public ``YYYY-NNNN`` case number.

        The user supplies the docket id directly (no enumeration). Invalid
        formats are dropped silently; the API never matches them anyway.
        """
        match = _DOCKET_ID_RE.match(docket_id or "")
        if not match:
            return
        year = int(match.group(1))
        number = int(match.group(2))
        yield self._build_case_request(year, number)

    @entry(OhioSupremeCourtDocket)
    def fetch_docket_speculative(
        self, case_id: YearlySpeculativeRange
    ) -> Request:
        """Speculative scan over a year's case-number sequence.

        Seed shape per year::

            {"fetch_docket_speculative": {"case_id":
                {"year": 2026, "min": 1, "soft_max": 600, "gap": 25}}}

        Year rollover is the operator's responsibility — add a new seed
        entry each January.
        """
        return self._build_case_request(case_id.year, case_id.min)

    # =========================================================================
    # Request builder
    # =========================================================================

    def _build_case_request(self, year: int, number: int) -> Request:
        docket_id = f"{year:04d}-{number:04d}"
        return Request(
            ...
            deduplication_key=f"ohio-case-{docket_id}",
        )
    ...
    def fails_successfully(self, response: Response) -> bool:
        ...
        searches. Real hits return a JSON object.
        """
    ...
    def parse_case_detail(
        ...
        docket_id = (
            ...
            or accumulated_data.get("docket_id")
            or ""
        ).strip()
        if not docket_id:
            return
        ...
        entries, entry_documents = _build_entries(
            docket_id, json_content.get("DocketItems") or []
        )
        decisions, decision_documents = _build_decisions(
            docket_id, json_content.get("DecisionItems") or []
        )
        issues = _build_issues(json_content.get("CaseIssues") or [])

        docket = OhioSupremeCourtDocket(
            docket_id=docket_id,
            court_id="ohio",
            date_filed=_parse_iso_date(case_info.get("DateFiled")),
            case_name=_clean_caption(case_info.get("Caption")) or docket_id,
            ...
            decisions=decisions,
            issues=issues,
            source_url=f"{BASE_URL}/#/caseinfo/{docket_id[:4]}/{docket_id[5:]}",
        )
    ...
    def _build_archive_request(
        ...
        docket_id, document_id, document_url, section = doc
        return Request(
            archive=True,
            request=HTTPRequestParams(
            ...
            deduplication_key=f"ohio-doc-{docket_id}-{document_id}",
        )
...
def _document_url(docket_id: str, document_name: str, section: str) -> str:
    ...
    subdirectory = f"{docket_id}\\{section}"
    return (
        f"{PDF_VIEWER_URL}"
...
def _build_parties(raw: list) -> list[OhioSupremeCourtParty]:
    ...
    for item in raw:
        ...
        out.append(
            OhioSupremeCourtParty(
                name=name,
                party_type=_clean(item.get("Type")) or "",
...
def _build_entries(
    ...
    for item in raw:
        ...
        document_url = (
            _document_url(docket_id, document_name, section)
            if document_name
            else None
        )
        entries.append(
        ...
        if document_name and document_url and item_id is not None:
            documents.append((docket_id, item_id, document_url, section))
...
def _build_decisions(
    ...
    for item in raw:
        ...
        document_name = _clean(item.get("DocumentName"))
        document_url = (
            _document_url(docket_id, document_name, "DecisionItems")
            if document_name
            else None
        )
        decisions.append(
        ...
        if document_name and document_url:
            ...
            if doc_id is not None:
                documents.append(
                    (docket_id, doc_id, document_url, "DecisionItems")
                )
