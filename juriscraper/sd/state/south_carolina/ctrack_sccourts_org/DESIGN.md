...
### Entry Points
- `@entry get_dockets()` — full-window: walks the most recent N days
  ending at today (uses scraper-level `default_period`).
- `@entry get_dockets_by_date(date_range: DateRange)` — explicit
  filed-date range across both courts.
- `@entry get_docket(docket_number: str)` — direct lookup. POSTs
  `csNumber=…` and follows the redirect to the case detail.

### Step Functions
```
get_dockets_by_date / get_dockets
   └─ POST caseSearch.do
...
### Models
Top-level data types yielded by the scraper: `SCAppDocket` and
`SCAppDocument`. Documents are emitted as their own records keyed back
to the parent docket via `(docket_id, event_id)`.

- `SCAppDocket` — top-level case record. Embeds parties + entries.
- `SCAppDocketEntry` — one row of the Event Information table.
  Records `event_id` (`deID`) so a downstream join can attach
  documents.
- `SCAppParty` — one row of the Party Information table.
- `SCAppDocument` — top-level. One per linked PDF.
