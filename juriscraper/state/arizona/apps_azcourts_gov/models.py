...
- ``arizctapp``: Arizona Court of Appeals (site code ``1CA``;
  Division One only — Division Two uses a different CMS, scraped by
  ``arizona/appeals2_az_gov``)
...
- ``AzAppDocument``: one record per archived PDF (→ CL ``RECAPDocument``).
...
BASE_URL: str = "https://apps.azcourts.gov/aacc/appella/"

# =========================================================================
# Per-court configuration
# =========================================================================
#
# CourtListener court id → site config. ``site_id`` is the token
...
COURTS: dict[str, dict] = {
    ...
    "arizctapp": {
        ...
        # separate publicaccess CMS (arizona/appeals2_az_gov).
...
class AzAppDocket(ScrapedData):
    ...
    only carries the metadata that appears on the case-list page. Maps to
    ...
    case_type: CleanString
    """One of the keys of ``COURTS[court]['case_types']``
    (``CR``, ``CV``, etc.)."""
...
class AzAppLowerCourtCase(ScrapedData):
    ...
    an Arizona appellate case. Maps to CourtListener
    ``OriginatingCourtInformation``."""

    court: str
    """The appellate court whose index this row came from."""

    lower_court_case_number: CleanString
    ...
    lower_court_name: CleanString
    """Section heading the row appeared under (e.g.
    ``COURT OF APPEALS, DIVISION ONE``,
    ``MARICOPA COUNTY SUPERIOR COURT``)."""

    lower_court_anchor: CleanString | None = None
