# Georgia Court of Appeals Scraper Design
...
| Entry | Param | Purpose |
|-------|-------|---------|
| `get_opinions_by_date` | `date_range: DateRange` | Date-range opinion search; yields one detail fetch + one PDF archive per row |
| `fetch_direct_appeal_docket` | `case_id: YearlySpeculativeRange` | Speculative `A{YY}A{NNNN}` lookup |
| `fetch_discretionary_application_docket` | `case_id: YearlySpeculativeRange` | Speculative `A{YY}D{NNNN}` lookup |
| `fetch_emergency_motion_docket` | `case_id: YearlySpeculativeRange` | Speculative `A{YY}E{NNNN}` lookup |
| `fetch_interlocutory_application_docket` | `case_id: YearlySpeculativeRange` | Speculative `A{YY}I{NNNN}` lookup |
| `fetch_original_proceeding_docket` | `case_id: YearlySpeculativeRange` | Speculative `A{YY}O{NNNN}` lookup |

The user asked for "two entry points, one for each, with the docket search
being speculative." Kent's speculation system iterates a single
`(year, integer)` axis per `@entry`, so to cover all five letter-prefixed
sequence spaces we register one speculative entry per letter. Conceptually
this is still "the docket search" — there is exactly one date-range entry
and one speculative family of entries.

### Step Functions

```
get_opinions_by_date ──▶ parse_opinion_search ──┬─▶ parse_case_detail ──▶ ParsedData(GaCoaDocket)
                                                 └─▶ download_opinion ──▶ ParsedData(GaCoaOpinion)

fetch_<letter>_docket ────────────────────────────▶ parse_case_detail ──▶ ParsedData(GaCoaDocket)
```

- `parse_opinion_search` reads the table on `docketdate/results_all.php`
  and dispatches: (a) a detail fetch for each case (deduped on case
  number), and (b) an archive request for each opinion PDF.
- `parse_case_detail` parses the per-case HTML — extracting the six
  sections above into a `GaCoaDocket` with nested entries / attorneys /
  trial-court info.
- `download_opinion` finalises the archived PDF as a `GaCoaOpinion`.

### Soft-404

`fails_successfully` returns `False` when the detail HTML contains
`<h2>Case Number: </h2>` (empty heading) — the universal soft-404 marker
for `results_one_record.php`.
...
- `GaCoaOpinion` — the archived opinion/order PDF (yielded as a separate
  top-level record so it can join back to the docket via `case_number`)
