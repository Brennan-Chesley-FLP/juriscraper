...
Entry points (§4)::
    ...
    - @entry dockets_by_bulk(court_ids: set[str],
                             argument_date?: DateRange,
                             decision_date?: DateRange)

``dockets_by_bulk`` is the canonical name for the enumeration entry:
Court-PASS only exposes the whole undecided-docket set (there is no
server-side date search), so the addressing mode is *bulk*. The
optional ``argument_date`` / ``decision_date`` windows are applied
**client-side** as the chain walks the results — per the §4
"Bulk + client-side filtering" rule the name stays ``*_by_bulk``.
...
Docket Lookup Flow — ``docket_by_number`` (emits NYCourtPassDocket)::

    1. docket_by_number → Docket.aspx
...
Docket Enumeration Flow — ``dockets_by_bulk`` (emits NYCourtPassDocket)::

    1. dockets_by_bulk → Docket.aspx
...
DOCKET_FORM = XPath("//form[@id='Form2']")
...
class NYCourtPassScraper(BaseScraper[_Yield]):
    ...
    last_verified: ClassVar[str] = "2026-06-17"
    ...
    def docket_by_number(
        ...
        Single-record direct lookup — per §4 it takes ``court_id: str``
        (exactly one court) rather than a ``court_ids`` set.
        """
        yield Request(
    ...
    def fill_docket_search(
        ...
        yield form.submit(
            ...
            submit_selector=CSS("input[name='ctl00$cphMain$btnFind']"),
    ...
    def parse_docket_results(
        ...
        # The gvResults grid must be present on every successful results
        # page — even the final page renders the table (with fewer rows).
        # Court-PASS intermittently returns the page chrome ("Search
        # Results:" header, "Return to Docket" button) with the results
        # region left as an empty <div></div> when an ASP.NET postback
        # doesn't fully populate the grid (observed mid-pagination: the
        # Page$N postback came back 200 with no grid at all). That page has
        # no rows to walk and no pagination links, so the step would
        # otherwise complete silently with zero children and stall
        # enumeration. Treat a missing grid as transient so the worker
        # refetches this page rather than dropping the rest of the results.
        grid = page.query(
            XPath("//table[contains(@id, 'gvResults')]"),
            "docket results grid",
            min_count=0,
        )
        if not grid:
            raise TransientException(
                "Court-PASS returned a results page with no gvResults grid "
                f"(page {expected_page}, url={response.url}); refetching."
            )

        pagination_rows = page.query(
            XPath(
                "//table[contains(@id, 'gvResults')]"
        ...
        for dv in DocketResultsParser()(page):
            ...
            yield form.submit(
                data={},
                submit_selector=CSS(f"#cphMain_gvResults_btnSelect_{i}"),
    ...
    # =========================================================================
    # Docket Entry Point Flow (Steps for docket_by_number)
    # =========================================================================
    ...
    def parse_docket_number_results(
        ...
        if not data_rows:
            call_details_button = page.query(
                XPath(
        ...
        yield form.submit(
            data={},
            submit_selector=CSS("#cphMain_gvResults_btnSelect_0"),
    ...
    def parse_docket_detail_for_entry(
        ...
        """Parse docket detail from the docket_by_number entry point.
    ...
    def _process_docket_detail_for_entry(
        ...
        """Shared docket-detail handling for the docket_by_number entry point.
        ...
        # instead, matching the dockets_by_bulk flow.
        form = page.find_form(DOCKET_FORM, "docket detail form")
        yield form.submit(
            data={
                "__EVENTTARGET": "ctl00$cphMain$bttnDetails",
    ...
    def _navigate_to_next_docket_page(
        ...
        yield form.submit(
            ...
            deduplication_key=(f"parse_docket_results:{target}"),
        )
