...
- ``get_supreme_dockets_by_date(date_range)``         — Indiana Supreme Court
- ``get_court_of_appeals_dockets_by_date(date_range)``— Indiana Court of Appeals
- ``get_tax_court_dockets_by_date(date_range)``       — Indiana Tax Court
- ``fetch_docket_by_number(case_number)``             — speculative single-case lookup
...
from .models import (
    ...
    COURT_ITEM_ID_COURT_OF_APPEALS,
    COURT_ITEM_ID_SUPREME,
    COURT_ITEM_ID_TAX,
    InAddress,
    InAttorney,
    InCrossReference,
...
class IndianaAppellateScraper(BaseScraper[InDocket | InDocument]):
    ...
    def get_supreme_dockets_by_date(
        ...
        """Indiana Supreme Court — date-based search."""
        yield self._build_search_request(
            court_item_id=COURT_ITEM_ID_SUPREME,
    ...
    def get_court_of_appeals_dockets_by_date(
        ...
        """Indiana Court of Appeals — date-based search.

        High-volume court: ~286 cases/month. Date ranges over ~3 months
        will hit the 1000-result cap and trigger automatic bisection.
        """
        yield self._build_search_request(
            court_item_id=COURT_ITEM_ID_COURT_OF_APPEALS,
            date_range=date_range,
            skip=0,
        )

    @entry(InDocket)
    def get_tax_court_dockets_by_date(
        self, date_range: DateRange
    ) -> Generator[Request, None, None]:
        """Indiana Tax Court — date-based search (very low volume)."""
        yield self._build_search_request(
            court_item_id=COURT_ITEM_ID_TAX,
            date_range=date_range,
            skip=0,
        )
    ...
    def fetch_docket_by_number(
        ...
        """Look up a case by exact docket number.
        ...
        yield Request(
            ...
            deduplication_key=f"in-bynum-{case_number}",
        )
    ...
    def parse_search_results(
        ...
        for hit in results:
            ...
            if not case_token or not case_number:
                continue
            court_id = COURT_CODE_TO_COURT_ID.get(court_code)
            ...
            yield Request(
                ...
                deduplication_key=f"in-case-{case_number}",
            )
                ...
                if full_url:
                    archive_reqs.append(
                        Request(
                            ...
                            deduplication_key=f"in-doc-{document_id}",
                        )
                    )
