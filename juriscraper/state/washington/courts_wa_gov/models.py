...
class WaBriefCase(ScrapedData):
    ...
    Maps to CourtListener ``Docket`` (with its briefs as
    ``DocketEntry``/``RECAPDocument`` rows)."""
    ...
    """Entry point used to reach this case."""
...
class WaDownloadedBrief(ScrapedData):
    ...
    ``(court, docket_number, hearing_date, brief_url)``.  Maps to
