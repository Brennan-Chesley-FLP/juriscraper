...
class LocalDevDriverDebugger:
    ...
    def _resolve_json_model(
        ...
        The dotted path is relative to the scraper's package directory.
        For example, "api.responses.PublicationsListResponse" resolves to
        the PublicationsListResponse class in the api.responses module
        relative to the scraper's package.
        ...
        # Resolve relative to scraper's package
    ...
    async def validate_structure(
        ...
        Discovers all steps with xsd or json_model metadata, retrieves
        their responses from the database, and validates each response
        against the corresponding spec.
        ...
        Example:
            ...
            for step in result['steps']:
                print(f"{step['continuation']}: {step['valid']}/{step['total_responses']}")
        """
        ...
        for spec in specs:
            ...
            # Count total responses for this continuation
            async with self.sql.db.execute(
                "SELECT COUNT(*) FROM responses WHERE continuation = ?",
                (continuation,),
            ) as cursor:
                row = await cursor.fetchone()
                total = row[0] if row else 0
            ...
            # Look up response_ids for invalid request_ids
            invalid_response_ids: list[int] = []
            if invalid_request_ids:
                placeholders = ",".join("?" * len(invalid_request_ids))
                async with self.sql.db.execute(
                    f"SELECT id FROM responses WHERE request_id IN ({placeholders})",  # noqa: S608
                    invalid_request_ids,
                ) as cursor:
                    rows = await cursor.fetchall()
                    invalid_response_ids = [r[0] for r in rows]
    ...
    async def validate_structure_detail(
        ...
        Runs the appropriate validation (XSD or JSON model) and returns
        the full error details rather than just pass/fail.
        ...
        Raises:
            ValueError: If neither request_id nor response_id is provided,
                or if the response/request cannot be found.
        """
        ...
        # Resolve to response record
        if response_id is not None:
            async with self.sql.db.execute(
                "SELECT id, request_id, continuation, content_compressed, compression_dict_id "
            ...
            async with self.sql.db.execute(
                "SELECT id, request_id, continuation, content_compressed, compression_dict_id "
                "FROM responses WHERE request_id = ? LIMIT 1",
                (request_id,),
            ) as cursor:
                row = await cursor.fetchone()
                if not row:
        ...
        # Load scraper and find the spec for this continuation
        metadata = await self.get_run_metadata()
        if not metadata:
            raise ValueError("No run metadata found")
        ...
        content = await decompress_response(self.sql.db, compressed, dict_id)
