# Iowa Appellate Courts Scraper Design
...
- `get_dockets_by_date(date_range: DateRange)` — incremental scrape via
  the advanced search. Splits the requested window into one-day slices to
  stay below the 2000-row server cap.
- `get_dockets()` — the parameterless variant uses the scraper's
  `date_filed` filter (last 24 h by default).
- `fetch_iowa_docket(case_id: YearlySpeculativeRange)` — backlog
  speculation, one probe per `YY-NNNN`. Goes straight to `AViewCase`.
...
entry
 └─ search submit (advanced)
    └─ parse_search_results (extracts unique docket IDs)
       └─ for each docket: fetch_case_summary (AViewCase)  ─┐
                                                            │ (chain
       speculative entry ───────────────────────────────────┘   merges)
       └─ parse_case_summary
          └─ fetch_long_title (AViewLongTitle)
             └─ parse_long_title
                └─ fetch_docket (AViewDocket)
                   └─ parse_docket_entries
                      └─ fetch_parties (AViewParties)
                         └─ parse_parties
                            └─ assemble_docket → ParsedData
```

### Models

- `IowaDocket` — main output (case summary + assembled fields)
- `IowaDocketEntry` — one register-of-actions row
- `IowaParty` — party or attorney row from the parties tab
