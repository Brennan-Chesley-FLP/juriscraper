"""Data models for the Minnesota P-MACS scraper."""
...
class MnDocument(ScrapedData):
    ...
    The P-MACS docket entry detail page exposes one or more
    ``/ctrack/document.do?document={hash}`` anchors; each becomes one
    of these records. The actual file is fetched by an
    ``archive=True`` Request whose deduplication key embeds the case
    number, ``deID``, and document hash."""
...
class MnOrcaInfo(ScrapedData):
    ...
    """``Appeal From:`` field (e.g. ``District Court``)."""
    ...
    """``Orig. Case Number:`` (the trial-court case number)."""
