...
``date_*`` prefix; ``case_name`` is the case caption.
"""
...
# CourtListener court id → the site's per-court CourtItemID used to scope a
# date-range search. The date entry fans out one search per requested court.
COURT_ID_TO_COURT_ITEM_ID: dict[str, int] = {
    "ind": COURT_ITEM_ID_SUPREME,
    "indctapp": COURT_ITEM_ID_COURT_OF_APPEALS,
    "indtc": COURT_ITEM_ID_TAX,
}
...
class InParty(ScrapedData):
    ...
    name: CleanString
    name_formatted: CleanString | None = None
    role: str | None = None
...
class InDocument(ScrapedData):
    ...
    docket_number: str
    court: str
    """CourtListener court id this document's docket belongs to."""
...
class InDocket(ScrapedData):
    ...
    court: str
    """CourtListener court id: one of ``ind`` / ``indctapp`` / ``indtc``."""
