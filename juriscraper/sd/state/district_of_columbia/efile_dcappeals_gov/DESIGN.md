...
[`juriscraper/sd/state/common/ctrack`](../../common/ctrack/__init__.py).
...
The scraper's `fails_successfully` checks for the marker substring;
the speculation/listing driver then suppresses the error and counts
the response as a miss.
...
- `get_dockets()` — convenience: walks the most recent 30 days using
  date-range search. For default scheduled runs.
- `get_dockets_by_date(date_range: DateRange)` — bulk scrape across an
  explicit filed-date window.
- `get_docket(docket_number: str)` — direct lookup by appellate case
  number (e.g. `26-CV-0339`). Posts the case-number search; the site
  302s on a single match straight to the detail page.

No speculative entry — date-search is the primary bulk strategy and
covers the same ground without the prefix-matrix complexity.
...
get_dockets / get_dockets_by_date
   └─ POST caseSearch.do
        └─ parse_search_listing
             ├─ for each row: GET caseView.do?csIID=N → parse_case_detail
...
get_docket(docket_number)
   └─ POST caseSearch.do (csNumber=…) follows 302
        └─ parse_case_or_miss
             ...
             └─ landed on caseView.do → parse_case_detail → (same fan-out as above)
```

### Models

- `DCAppDocket` — top-level case record. Embeds parties + entries.
- `DCAppDocketEntry` — one row of the Events table; records `event_id`
  (the C-Track `deID`) for downstream document join.
- `DCAppParty` — one row of the Party Information table, capturing the
  6-column shape (incl. IFP, arguing attorney, e-filer flags).
- `DCAppDocument` — top-level. One per archived PDF, joined back to
  the parent docket via `(docket_id, event_id)`.
...
parser, and the soft-404 marker. SC was migrated to consume the same
helpers in this PR.
