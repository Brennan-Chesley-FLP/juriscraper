# Texas Appellate Courts (TAMES) Scraper Design
...
- `@entry get_dockets_by_date(date_range: DateRange)` — bulk walk of the
  Date-Filed search for the given window (all 17 court checkboxes ticked)
- `@entry fetch_docket(docket_number: str)` — single-case direct lookup,
  bypassing search via `Case.aspx?cn={docket_number}`. TAMES resolves the
  court from the docket number prefix server-side and 302s to the
  canonical URL with the right `coa=`, so we don't pre-compute it.
...
```
entry (date_range or docket_number)
  → fetch_search_form         GET /CaseSearch.aspx     [date_range only]
  → submit_search             POST with hidden fields + date range + all 17 checkboxes
  → parse_search_results      walk rgRow / rgAltRow rows
  ...
  → parse_case_detail         read coa= from response.url, route to legacy
                              parser (SC / CCA / COA), emit ParsedData(TexasDocket)
                              and one archive=True Request per attached document
  → handle_document_download  emit ParsedData(TexasDocument) with local_path set
```
